...
target = 50000
filename = "IMDBMovieDataset.csv"
start_index = 1  # last accessed movie is at 6379
style.init()
...
def getPoster(link):
    ...
    soup = BeautifulSoup(req.text, 'html.parser')
    movie_poster = str(soup.find('div', attrs={'class': 'sc-292689f9-1 bQplXF media-viewer'}).find('div', attrs={
...
def extract(way):
    ...
    if way == "continous":
        ...
        url = "https://www.imdb.com/list/ls041437328/?sort=moviemeter,asc&st_dt=&mode=detail&page=1"
        req = requests.get(url, headers)
        soup = BeautifulSoup(req.text, 'html.parser')
